...
var targetScores = [930,1000,1000,1070,1145,1170,1230,1200,1255,1400,1250,1485,1200,1500,1540];
...
var upgradePrice3 = [1000,1200,1500];
var truckUpgrade = [0,0,0]; // motors,riepas,karba
...
exportRoot.menuB.y = 191 + (stage.y / stage.scaleY);
...
axle1.CreateFixture(boxDef);
         ...
         circleDef.friction = 1 + (truckUpgrade[1] * 1.3);//1.6
...
function drive() {
		 ...
		 motor1.SetMotorSpeed((18 + (truckUpgrade[2] * 2.5))*Math.PI * (down ? -.3 : up ? .3 : 0));
         motor1.SetMaxMotorTorque(down || up ? 18 + (truckUpgrade[0] * 2.5) : 0.5);
		 
		 motor2.SetMotorSpeed((18 + (truckUpgrade[2] * 2.5))*Math.PI * (down ? -.2 : up ? .2 : 0));
         motor2.SetMaxMotorTorque(down || up ? 10 + (truckUpgrade[0] * 1.3) : 0.5);
...
if (engineSound) engineSound.stop();
	...
	if(levelScores[Level -1] == 0) moneyBonus = 300 + (Level * 10);
	levelMoney = Math.floor((levelScore / 8) + moneyBonus);
	nauda += levelMoney;
...
function readMemory() {
	for (i = 0; i < levelScores.length; i++) {
		if(localStorage.getItem('levelscoretest' + i) == null) localStorage.setItem('levelscoretest' + i, 0);
    levelScores[i] = parseInt(localStorage.getItem('levelscoretest' + i));
	}
	
	if(localStorage.getItem('naudatest') == null) localStorage.setItem('naudatest', 0);
	nauda = parseInt(localStorage.getItem('naudatest'));
	
	if(localStorage.getItem('motortest') == null) localStorage.setItem('motortest', 0);
	truckUpgrade[0] = parseInt(localStorage.getItem('motortest'));
	if(localStorage.getItem('riepastest') == null) localStorage.setItem('riepastest', 0);
	truckUpgrade[1] = parseInt(localStorage.getItem('riepastest'));
	if(localStorage.getItem('karbatest') == null) localStorage.setItem('karbatest', 0);
	truckUpgrade[2] = parseInt(localStorage.getItem('karbatest'));
	...
	// reset memory
	//for (i = 0; i < levelScores.length; i++) {
	//localStorage.setItem('levelscoretest' + i, 0);
	//}
	//localStorage.setItem('naudatest', 0);
}
// ------------------------------------------------------------------------------
function writeMemory() {
		for (i = 0; i < levelScores.length; i++) {
    localStorage.setItem('levelscoretest' + i, levelScores[i]);
	}
	
	localStorage.setItem('naudatest', nauda);
	localStorage.setItem('motortest', truckUpgrade[0]);
	localStorage.setItem('riepastest', truckUpgrade[1]);
	localStorage.setItem('karbatest', truckUpgrade[2]);
