...
var targetScores = [801,813,847,912,937,968,943,1041,1091,1055];
...
exportRoot.virsraksts.y = 9 - (stage.y / stage.scaleY);
exportRoot.virsraksts.x = 11 -(stage.x / stage.scaleX);
}
...
//exportRoot.addChild(smallcanvas);
	...
	bitmap2.y = -89;
	bitmap2.scaleX = 50 / 107; //bitmap2.image.naturalWidth;
...
function drive() {
	...
	exportRoot.level.car.x += increment * Math.sin(exportRoot.level.car.rotation * Math.PI / 180);
    exportRoot.level.car.y -= increment * Math.cos(exportRoot.level.car.rotation * Math.PI / 180);
...
function enemydrive() {	
		...
		enemy.x += enemySpeed * Math.sin(enemy.rotation * Math.PI / 180);
		enemy.y -= enemySpeed * Math.cos(enemy.rotation * Math.PI / 180);
...
function readMemory() {
	...
	for (i = 0; i < levelScores.length; i++) {
		if(localStorage.getItem('scorepolice' + i) == null) localStorage.setItem('scorepolice' + i, 0);
    levelScores[i] = parseInt(localStorage.getItem('scorepolice' + i));
	}
	updateAchievements();
}
// ------------------------------------------------------------------------------
function writeMemory() {
		for (i = 0; i < levelScores.length; i++) {
    localStorage.setItem('scorepolice' + i, levelScores[i]);
	}
	updateAchievements();
}
